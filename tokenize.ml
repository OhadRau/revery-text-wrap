...
let split_on_newline string =
  ...
  for i = 0 to String.length string - 1 do
    if string.[i] = '\n' then begin
      Queue.add (String.slice ~first:!first ~last:i string)lines;
      first := i + 1
    end
  done;
  (* Append the remaining characters after the last split *)
  if !first < String.length string - 1 then begin
    Queue.add (String.slice ~first:!first ~last:(String.length string) string) lines 
  end;
  lines
  ...
  for i = 0 to String.length string - 1 do
    ...
    | '\n' | ' ' | '\t' ->
      ...
      if !first <> i then begin
        Queue.add (String.slice ~first:!first ~last:i string) tokens
      end;
      (* Then push this character *)
      Queue.add (String.slice ~first:i ~last:(i+1) string) tokens;
      (* And skip it *)
      first := i + 1
    | '-' ->
      ...
      Queue.add (String.slice ~first:!first ~last:(i+1) string) tokens;
      first := i + 1
    | _ -> ()
  done;
  (* Append the remaining characters after the last split *)
  if !first < String.length string - 1 then begin
    Queue.add (String.slice ~first:!first ~last:(String.length string) string) tokens
  end;
  tokens
